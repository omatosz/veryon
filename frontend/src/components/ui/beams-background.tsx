...
// Faixa de matiz dos feixes: ciano (~190) ate violeta (~260), ecoando a
// paleta do projeto (primary #1EC1CB fica bem no inicio dessa faixa).
const HUE_MIN = 190
const HUE_RANGE = 70
const OPACITY_BY_INTENSITY = { subtle: 0.7, medium: 0.85, strong: 1 } as const
...
export function BeamsBackground({ className, children, intensity = 'strong' }: BeamsBackgroundProps) {
  ...
  useEffect(() => {
    ...
    function resetBeam(beam: Beam, index: number, totalBeams: number) {
      ...
      beam.hue = HUE_MIN + (index * HUE_RANGE) / totalBeams
    ...
    function drawBeam(ctx: CanvasRenderingContext2D, beam: Beam) {
      ...
      const pulsingOpacity = beam.opacity * (0.8 + Math.sin(beam.pulse) * 0.2) * OPACITY_BY_INTENSITY[intensity]
